...
export default function Header() {
  ...
  return (
    <header className="flex items-center h-16 px-4 border-b shrink-0 md:px-6 justify-between">
      ...
      <SheetContent side="left" className="block md:hidden bg-white p-0">
          ...
          {navItems.map((navItem, idx) => (
            <Link
              ...
              className={`h-full relative flex items-center whitespace-nowrap rounded-md ${
                navItem.active
                  ? 'font-base text-xl bg-gray-200 shadow-sm text-neutral-700'
                  : 'hover:bg-gray-100 hover:text-neutral-700 text-neutral-500'
              }`}
            >
              <div className="relative font-base text-xl py-1.5 px-2 flex flex-row items-center space-x-2 rounded-md duration-100">
                {navItem.icon}
